...
const FavoritesEl = ({ adverts }) => {
  ...
  const favoriteAdverts = adverts.filter((el) => favorites.includes(el.id));
  const showLoadMore = visibleItems < favoriteAdverts.length;
  ...
  return (
    <>
      <Container>
      <ul className={s.list}>
        {favoriteAdverts.slice(0, visibleItems).map((el) => (
